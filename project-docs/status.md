...
- P73 desktop UI parity correction 已完成验证：P72 被用户指出仍未完美复刻且误把移动端作为目标；P73 明确 Rust 主线 UI 是桌面应用 UI，移动端不是验收目标。Rust/Leptos UI 现在按当前前端桌面形态收紧：顶部动作和 5 个 icon tab 使用图标样式，Dashboard 去掉错误的“选择当前提供商”标题并恢复 provider switch-board + 继续添加提供商区，Provider 卡片和预设卡片使用当前 provider assets，Add Provider 使用空表单初始态、双栏快捷预设、红框第三方兼容接口、行式模型映射和橙色“一键应用到 Claude 桌面版”按钮。桌面 Playwright 截图保存在 `target/ui-smoke/p73/`，覆盖 `dashboard-desktop-r3.png` 和 `provider-add-desktop-r2.png`；mocked Tauri bridge 下 console 为 0 error / 1 个既有 Trunk/SRI warning；不再跑移动端截图作为验收项。P73 本地已通过 `cargo fmt --all -- --check`、`trunk build --release`、`cargo test --workspace`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo tauri build`、`cargo xtask verify --stage rc-readiness`。P73 已 push 为 commit `941798a`，non-publishing macOS workflow run `25603881072` 已通过 arm64/x64 Rust gate、Leptos build、Tauri build、bundle/pkg smoke、macOS real Desktop local config smoke；artifacts 已下载到 `target/github-actions/25603881072` 并刷新 macOS evidence handoff。
- P74 UI / installer identity / auth scheme / tray icon root-cause pass 已完成本地验证：只读复核当前截图、旧 Tauri 版本和 Python 稳定线后确认，UI 漂移来自 P71-P73 多层覆盖和没有把当前桌面前端截图作为 contract；安装目录不能继承来自 Rust 主线 `identifier` 改成 `app.ccdesktopswitch.mainline` 且缺旧 NSIS per-machine template/hooks；已保存 Provider 信息不能完整继承的风险来自 `authScheme` 只迁移到 config 但没有贯穿 Provider summary/save/gateway headers；任务栏/托盘透明缩略图来自 tray 没有绑定 app 默认 icon。P74 已重建 desktop-only Leptos UI 合同、恢复稳定 app identity `io.github.lonr6.ccdesktopswitch`、接回旧 NSIS install location hooks、贯穿 Provider `AuthScheme`，并给 tray 绑定 id/tooltip/default icon。Playwright 桌面截图保存在 `target/ui-smoke/p74/`，覆盖 Dashboard、Provider、Add Provider、Proxy、Settings、Guide；本地已通过 `cargo fmt --all -- --check`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo test --workspace`（111 passed, 2 ignored real Desktop smoke）、`trunk build --release`、`cargo tauri build`、`cargo xtask verify --stage app-shell`、`cargo xtask verify --stage rc-readiness`。
- P75 frontend parity / scroll / installer pass 已完成本地验证：只读核对旧 Tauri worktree 最新 `frontend` 后确认，P74 仍缺部分当前前端按钮、跳转和 command-backed flow；滚动失效来自 `html, body { overflow: hidden; }` 下 `.app-shell` 只设 `min-height: 100vh`，没有固定高度滚动容器；NSIS `.exe` 不能继承 MSI 安装目录来自旧恢复逻辑没有扫描 MSI/GUID uninstall key。P75 已把 app shell 改为固定 `height: 100vh` flex + `.app-main` 内部滚动，补入 redacted config snapshot、settings read/update、proxy status/logs/clear、manual backup、clipboard copy、`configure_desktop` 兼容 alias，并把更多 Provider/Proxy/Settings/Model mapping 控件接到真实 Rust command；NSIS 现在扫描 HKLM/HKCU uninstall entries 的 `DisplayName == CC Desktop Switch`。本地已通过 `cargo fmt --all -- --check`、`trunk build --release`、`cargo check --workspace`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo test --workspace`（111 passed, 2 ignored real Desktop smoke）、Playwright mocked desktop scroll smoke、`cargo tauri build`、`cargo xtask verify --stage rc-readiness`。
- P75 post-push CI follow-up 已完成验证：macOS run `25722210583` 的 arm64 job 在 `cargo test --workspace` 暴露 `apply_flow_fixture_blocks_port_conflict_before_write` 临时路径同毫秒碰撞假设；测试已改为断言 `_meta.json` 和 `cc-desktop-switch-local-gateway.json` 未写入，而不是要求 Desktop root 目录不存在。本地已补跑目标测试、`cargo test --workspace` 和 `cargo clippy --workspace --all-targets -- -D warnings`。follow-up run `25724455576` 已通过 macOS arm64/x64 Rust gate、Leptos build、Tauri build、bundle/pkg smoke、macOS real Desktop local config smoke，并生成 P75 macOS platform / real Desktop smoke handoff；`cargo xtask verify --stage rc-readiness` 已识别这些 handoff 并通过。
- RC1 readiness audit 已写入 `project-docs/handoff/2026-05-09-rc1-readiness-audit.md`；P75 已刷新 macOS arm64/x64 workflow 和 macOS real Desktop smoke evidence。进入 RC 前仍需基于 P75 包重新执行人工 Windows upgrade smoke；人工测试通过前不要打 tag、不要 GitHub Release、不要更新 `latest.json`。
...
- Pure Rust UI spike 的核心 command bridge / build / package 已通过；UI 已能触发 provider CRUD/reorder/import/export/save-as/preset-import、model mapping edit、config backup readback/create、health、gateway status-start-stop、proxy logs clear/read、settings save、desktop config probe、apply dry-run/apply、diagnostics summary/package/save-as；P75 后 UI 合同以旧 Tauri 最新 `frontend` 和用户截图中的当前 CC Desktop Switch 桌面布局为准，并吸收 CC Switch 的紧凑 provider card、空状态和 icon 导航密度；移动端不是验收目标。仍缺 update check/install、feedback submit、CC-Switch auto-detect/import list、provider usage query、saved-provider-specific speed test、model fetch/autofill、restart Claude Desktop、clear Desktop config 等完整 action parity。
- macOS 1M / configLibrary 问题需要最终真人实机复核；P75 follow-up run `25724455576` 已刷新 macOS arm64/x64 workflow 和 real Desktop smoke pass evidence，macOS 安装包已下载到本机供人工测试。
- Windows 更新安装器不弹出问题需要能复现的 Windows 11 更新路径验证；P75 已恢复旧 app identity、旧 NSIS install location hooks，并扩展到 MSI/GUID uninstall key 扫描；真实“覆盖旧版本安装后继承旧目录/旧配置/任务栏图标”仍需人工或 VM 执行安装包验证。
...
1. 用桌面 `CCDS-P75-Manual-Test-20260512` 里的 P75 Windows NSIS/MSI 包执行人工 upgrade smoke：旧安装目录是否被预选、`~/.cc-desktop-switch/config.json` 是否继承、Provider `authScheme` 是否保留、任务栏/托盘图标是否显示。
2. 执行下一轮 P76 UI action parity：update check/install、feedback、CC-Switch detect/import、provider usage、saved-provider test、model fetch/autofill、restart/clear Desktop config。
3. 按 `project-docs/runbooks/final-human-rc-smoke.md` 用桌面测试目录中的 Windows/macOS 包执行最终人工测试：Windows x64、macOS arm64、macOS x64。
4. 人工测试通过后，再决定是否进入 release metadata / signing / publish 准备；当前仍不要打 tag、不要 GitHub Release、不要更新 `latest.json`。
...
- P74 UI / installer / tray / authScheme 根因记录：`project-docs/bugs/2026-05-12-p74-ui-installer-icon-authscheme-root-cause.md`
- P74 阶段总结：`project-docs/handoff/2026-05-12-p74-ui-installer-identity-icon-authscheme-summary.md`
- P75 frontend parity / scroll / installer 根因记录：`project-docs/bugs/2026-05-12-p75-frontend-scroll-installer-parity-root-cause.md`
- P75 阶段总结：`project-docs/handoff/2026-05-12-p75-frontend-parity-scroll-installer-summary.md`
- P75 macOS platform smoke evidence：`project-docs/handoff/2026-05-12-p75-macos-platform-smoke-evidence-summary.md`
- P75 macOS real Desktop smoke evidence：`project-docs/handoff/2026-05-12-p75-macos-real-desktop-smoke-evidence-summary.md`
