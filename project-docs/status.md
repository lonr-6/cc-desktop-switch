...
- RC1 readiness audit 已写入 `project-docs/handoff/2026-05-09-rc1-readiness-audit.md`；P69/P73 后曾补齐 macOS arm64/x64 workflow 和 macOS real Desktop smoke evidence。P74 改动了 UI、installer identity、tray icon 和 auth scheme，进入 RC 前仍需基于 P74 包重新执行人工 Windows upgrade smoke，并在 push 后刷新 non-publishing macOS arm64/x64 workflow evidence。
...
- Pure Rust UI spike 的核心 command bridge / build / package 已通过；UI 已能触发 provider CRUD/reorder/import/export/save-as/preset-import、model mapping edit、config backup readback、health、gateway status-start-stop、desktop config probe、apply dry-run/apply、diagnostics summary/package/save-as；P74 后 UI 合同以用户截图中的当前 CC Desktop Switch 桌面布局为准，并吸收 CC Switch 的紧凑 provider card、空状态和 icon 导航密度；移动端不是验收目标。app shell 已接入 single-instance、tray close-to-hide、tray default icon 和 installer identity 静态门禁；仍需真人验证 Windows in-place upgrade、任务栏图标和旧配置继承。
- macOS 1M / configLibrary 问题需要 macOS 1.6259.1+ 实机验证；P69/P73 已有 macOS arm64/x64 workflow 和 real Desktop smoke pass evidence，但 P74 改了 `tauri.conf.json`、NSIS、tray 和 UI，RC 前需要重新 push 后触发 non-publishing macOS arm64/x64 workflow，不能复用旧 artifacts 当作最终包证据。
- Windows 更新安装器不弹出问题需要能复现的 Windows 11 更新路径验证；P74 已恢复旧 app identity 和 NSIS install location hooks，真实“覆盖旧版本安装后继承旧目录/旧配置/任务栏图标”仍需人工或 VM 执行安装包验证。
...
1. 用 P74 Windows NSIS/MSI 包执行人工 upgrade smoke：旧安装目录是否被预选、`~/.cc-desktop-switch/config.json` 是否继承、Provider `authScheme` 是否保留、任务栏/托盘图标是否显示。
2. P74 commit push 后触发 non-publishing macOS arm64/x64 workflow，下载 artifacts 并刷新 platform / real Desktop smoke handoff。
3. 按 `project-docs/runbooks/final-human-rc-smoke.md` 执行最终人工测试：Windows x64、macOS arm64、macOS x64。
4. 人工测试通过后，再决定是否进入 release metadata / signing / publish 准备；当前仍不要打 tag、不要 GitHub Release、不要更新 `latest.json`。
5. 用 release manifest gate 和 directory verifier 复核未来 release staging 中的 `latest.json` 引用、sha256、sig、public key 和平台资产完整性。
