...
- P75 post-push CI follow-up 已完成本地验证：macOS run `25722210583` 的 arm64 job 在 `cargo test --workspace` 暴露 `apply_flow_fixture_blocks_port_conflict_before_write` 临时路径同毫秒碰撞假设；测试已改为断言 `_meta.json` 和 `cc-desktop-switch-local-gateway.json` 未写入，而不是要求 Desktop root 目录不存在。本地已补跑目标测试、`cargo test --workspace` 和 `cargo clippy --workspace --all-targets -- -D warnings`。
- RC1 readiness audit 已写入 `project-docs/handoff/2026-05-09-rc1-readiness-audit.md`；P69/P73 后曾补齐 macOS arm64/x64 workflow 和 macOS real Desktop smoke evidence。P75 改动了 UI、NSIS installer 和 command surface，进入 RC 前仍需基于 P75 包重新执行人工 Windows upgrade smoke，并在 push 后刷新 non-publishing macOS arm64/x64 workflow evidence。
...
- macOS 1M / configLibrary 问题需要 macOS 1.6259.1+ 实机验证；P69/P73 已有 macOS arm64/x64 workflow 和 real Desktop smoke pass evidence，但 P75 改了 UI、NSIS installer 和 command surface，RC 前需要重新 push 后触发 non-publishing macOS arm64/x64 workflow，不能复用旧 artifacts 当作最终包证据。
...
1. 用 P75 Windows NSIS/MSI 包执行人工 upgrade smoke：旧安装目录是否被预选、`~/.cc-desktop-switch/config.json` 是否继承、Provider `authScheme` 是否保留、任务栏/托盘图标是否显示。
2. P75 commit push 后触发 non-publishing macOS arm64/x64 workflow，下载 artifacts 并刷新 platform / real Desktop smoke handoff。
3. 执行下一轮 P76 UI action parity：update check/install、feedback、CC-Switch detect/import、provider usage、saved-provider test、model fetch/autofill、restart/clear Desktop config。
4. 按 `project-docs/runbooks/final-human-rc-smoke.md` 执行最终人工测试：Windows x64、macOS arm64、macOS x64。
5. 人工测试通过后，再决定是否进入 release metadata / signing / publish 准备；当前仍不要打 tag、不要 GitHub Release、不要更新 `latest.json`。
