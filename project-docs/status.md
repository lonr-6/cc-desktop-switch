...
- P69 第四轮远端 macOS workflow 已通过 Rust gate、Leptos build、Tauri build、bundle smoke、DMG verify 和 PKG create/expand，但两条 runner 都在 real Desktop smoke 返回 `result=Fail`，且失败时 artifact upload 被跳过，无法取得 cargo test log；已在临时 `HOME` 下保留原 `CARGO_HOME`/`RUSTUP_HOME`，并让失败也上传 real-smoke evidence/log。仍需重新 push 触发 workflow。
...
1. 推送 P69 macOS real smoke 临时 `HOME` / evidence 保留修复提交，只触发 non-publishing `Rust Mainline Platform Smoke` workflow，不打 tag、不触发 `Release`；workflow 通过后下载 `rust-mainline-macos-arm64` 和 `rust-mainline-macos-x64` artifacts。
2. 对下载后的 artifacts 运行 `scripts/macos/Collect-PlatformSmokeEvidence.ps1` 生成 macOS arm64/x64 combined pass handoff；再运行 `scripts/macos/Collect-RealDesktopSmokeEvidence.ps1` 生成 macOS real Desktop smoke pass handoff。
3. 如继续做 preset marketplace，需要在 P42 离线 manifest/hash gate 之上补签名信任链、远端 fetch 策略和 UI trust wording；不要直接启用联网拉取。
4. 等真实 macOS artifacts 都产出后，用 release manifest gate 和 directory verifier 复核 `latest.json` 引用、sha256、sig、public key 和平台资产完整性。
