...
1. 进行 Rust/Leptos UI/UX 一致性收尾：按 `docs/product/ui-ux-rust-mainline.md` 复核旧 CC Switch 工作流雏形、readiness dashboard、Provider/Gateway/Apply/Diagnostics 主路径，优先做小范围可验证优化，不引入手写 JS 业务逻辑。
2. 准备最终人工测试包：Windows x64 和 macOS arm64/x64 的安装、启动、Provider 保存、gateway smoke、Apply、Claude Desktop safe route readback、tray/single-instance、diagnostics/report issue 检查表。
