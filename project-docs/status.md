...
1. 推送 P67 macOS `.icns` icon 修复提交，只触发 non-publishing `Rust Mainline Platform Smoke` workflow，不打 tag、不触发 `Release`；workflow 通过后下载 `rust-mainline-macos-arm64` 和 `rust-mainline-macos-x64` artifacts。
