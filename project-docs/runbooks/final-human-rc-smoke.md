...
4. Save a Provider with an API key.
5. Run Health.
6. Run Gateway smoke.
7. Run Apply.
8. Restart Claude Desktop.
9. Confirm Claude Desktop model routes are `claude-*` safe routes.
10. Confirm `Default` is absent from the Claude Desktop model menu.
11. Confirm no raw upstream model name is visible in Claude Desktop.
12. Close the CC Desktop Switch window and confirm the app hides to tray.
13. Launch the app again and confirm the existing instance is restored.
14. Export diagnostics package and confirm it is redacted.
15. Open issue draft and confirm it contains no API key, gateway key, Authorization header, cookie, or URL token.
...
3. Repeat the Windows checklist items 3 through 15.
