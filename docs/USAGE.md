...
macOS 用户可以下载对应的 DMG 或 PKG。DMG 打开后把应用拖到“应用程序”；如果该位置已有旧版，Finder 会提示是否替换。PKG 会安装到 `/Applications/CC Desktop Switch.app`，再次安装或安装新版本时会替换该位置的旧应用。
...
在 Windows 上，本工具会写入 Claude Desktop 使用的本机策略配置。在 macOS 上，本工具会写入 Claude Desktop 的 3P 本地配置，并自动定位当前生效的 `configLibrary` 配置条目。
...
### macOS 上为什么不需要手动找配置文件？

Claude Desktop 的 macOS 3P 配置可能同时存在于根配置文件和 `configLibrary` 条目中。`configLibrary` 条目的文件名是每台设备生成的 ID，不应该手动照抄别人的文件名。

本工具会读取 Claude Desktop 的当前配置索引，自动定位正在生效的配置条目并写入对应内容。
