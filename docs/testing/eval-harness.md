...
| `ui.current_frontend_parity` | User opens the Rust UI after the P74 screenshot-driven correction | Header icon tabs, provider switch cards, provider assets, Add Provider two-column flow, red compatibility box, row-style model mapping, Proxy, Settings, Guide, and CC Switch-style empty/provider card density follow the current desktop frontend baseline; mobile is not an acceptance target | ui/manual |
...
| `ui.current_frontend_parity` | `trunk build --release`; Playwright at `http://127.0.0.1:1425/?p74=...` | P74 supersedes P73 as the UI baseline. Browser smoke covers desktop Dashboard, Provider, Add Provider, Proxy, Settings, and Guide. Mobile is not an acceptance target for this desktop app. Screenshots are under `target/ui-smoke/p74/` with `*-final2.png` names. Console result with mocked Tauri bridge: 0 errors, 1 existing Chromium/Trunk SRI warning. Current frontend markers verified: five icon tab shell, provider switch cards with provider assets, CC Switch-style empty state/card density, Add Provider two-column form, red third-party compatibility box, row-style model mapping, orange one-click apply button, Settings rows, Proxy status/log/metrics, and Guide steps |
...
| `rc1.readiness_incomplete` | `cargo xtask verify --stage rc-readiness` | P74 local run passes against existing handoff evidence and now also statically checks stable installer identity, NSIS hooks, and tray icon binding. Because P74 changes package/UI artifacts after the previous macOS workflow, fresh non-publishing macOS workflow evidence is still required before RC packaging even if the static audit command passes |
...
| `installer.old_dir_not_detected` | `cargo xtask verify --stage rc-readiness`; Windows manual upgrade smoke | P74 static gate covers stable Tauri identifier `io.github.lonr6.ccdesktopswitch`, per-machine NSIS mode, old NSIS template/hooks, and install-location hook files. Real old-directory selection still requires a Windows in-place installer run because it depends on the old uninstall registry `InstallLocation` |
| `platform.macos_arm64_x64_smoke_path` | `.github/workflows/rust-mainline-platform-smoke.yml`, `scripts/macos/Collect-PlatformSmokeEvidence.ps1`, and `project-docs/runbooks/macos-rust-mainline-smoke.md` | Workflow path targets `macos-14` arm64 and `macos-15-intel` x64; P69 and P73 produced pass evidence for prior commits. P74 changes package/UI artifacts, so this workflow must be rerun after pushing P74 before final RC evidence is current |
| local full gate | `cargo xtask verify --all` or focused equivalent commands | Runs fmt, workspace tests, clippy, UI release build, and Tauri build on the current platform. P74 focused local run passed `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace` with 111 passed and 2 ignored real Desktop smoke tests, `trunk build --release`, and `cargo tauri build` producing Windows MSI/NSIS bundles |
