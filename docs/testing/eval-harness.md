...
| `ui.current_frontend_parity` | `trunk build --release`; Playwright at `http://127.0.0.1:1422/?p72=...` | P72 supersedes P71 as the UI baseline. Browser smoke covered desktop `1366x900` and mobile `390x900`, including Dashboard and Add Provider. Screenshots are under `target/ui-smoke/p72/`. Console result: 0 errors, 1 existing Chromium/Trunk SRI warning; no horizontal overflow; current frontend markers verified: five icon tab shell, provider switch cards, red third-party compatibility box, and orange one-click apply button |
