...
| `app.tray_close_behavior` | `cargo xtask verify --stage app-shell`; latest Windows packaged app smoke | Covers compile-time registration of Tauri tray icon support, close-request hide handler, tray show action, left-click restore, and explicit quit action; P43 latest packaged smoke verified close hides the `CC Desktop Switch` main window while the process remains alive and third launch restores it |
| config migration | `cargo xtask verify --stage config` | Covers Python stable config load, backup-before-save, `providerId`, and route identity preservation |
...
| `ui.current_frontend_parity` | `trunk build --release`; Playwright at `http://127.0.0.1:1424/?p73=...` | P73 supersedes P72 as the UI baseline. Browser smoke covers desktop `1366x900` Dashboard and Add Provider only; mobile is not an acceptance target for this desktop app. Screenshots are under `target/ui-smoke/p73/`. Console result with mocked Tauri bridge: 0 errors, 1 existing Chromium/Trunk SRI warning. Current frontend markers verified: five icon tab shell, provider switch cards with provider assets, Dashboard continue-add-provider section, Add Provider two-column form, red third-party compatibility box, row-style model mapping, and orange one-click apply button |
...
| `rc1.readiness_incomplete` | `cargo xtask verify --stage rc-readiness` | P64 local run includes a new static pass check proving the macOS workflow runs real Desktop local config smoke and uploads its evidence/log. The stage still exits non-zero as expected because macOS arm64/x64 workflow smoke and macOS real Desktop smoke handoff evidence are still missing. P58 tightened final handoff matching to require collector-style fingerprints, test names, logs, workflow run fields, and artifact fields |
...
| `platform.macos_arm64_x64_smoke_path` | `.github/workflows/rust-mainline-platform-smoke.yml`, `scripts/macos/Collect-PlatformSmokeEvidence.ps1`, and `project-docs/runbooks/macos-rust-mainline-smoke.md` | Workflow path exists and YAML parses locally. It targets `macos-14` arm64 and `macos-15-intel` x64, matching the current GitHub-hosted runner reference; checks `uname -m`; runs Rust/UI/Tauri gates; verifies app bundle/DMG; creates PKG; expands PKG; uploads non-release workflow artifacts plus `platform-smoke-evidence.md`; P64 also runs macOS real Desktop local config smoke under a temporary `HOME` and uploads `real-desktop-smoke` evidence/log files inside each artifact. The workflow is covered by `rc-readiness` static checks for trigger, architecture, gates, bundle smoke, artifact name, retention, real-smoke step, and collectors. P63 read-only GitHub check showed this workflow was not available remotely on default; after pushing branch and opening draft PR #21, `workflow_dispatch` still returned 404 and PR checks were empty, so P65 adds a `push` trigger limited to `codex/**` branches and relevant paths. Real workflow execution evidence is still missing |
| local full gate | `cargo xtask verify --all` | Runs fmt, workspace tests, clippy, UI release build, and Tauri build on the current platform. P62 local run passed on Windows x64 with 110 unit tests passed, 2 real Desktop smoke tests ignored, `trunk build --release` passed, and `cargo tauri build` produced MSI and NSIS bundles |
