...
| `apply.dry_run_secret_boundary` | UI asks for Apply dry-run evidence | Dry-run output can describe local gateway topology but must not return the real config-scoped gateway key through the Tauri/UI payload | apply-flow |
...
| `gateway.openai_stream_semantic_conversion` | OpenAI Chat SSE emits `chat.completion.chunk` frames | Gateway returns Anthropic-style SSE events, preserves tool-call deltas, and exposes only the safe route model | gateway |
| `gateway.auth_required` | A local process calls `/v1/models` or `/v1/messages` without the configured gateway key | HTTP 401 with structured auth error, stats/logs record the failure, and no key value is logged | gateway |
| `gateway.unmapped_model_route` | Claude Desktop requests an unmapped route | HTTP 400, no fallback to `Default`, and response/runtime log route fields do not echo raw-looking request model text | gateway |
...
| `provider.crud_reorder_roundtrip` | Provider is edited, reordered, deleted, active provider changes, or gateway port changes | Config roundtrip preserves provider IDs, hides secrets in summaries, enforces exact reorder sets, and refreshes the running gateway after provider/model/settings/import/delete/settings-port mutations | provider-parity |
...
| `provider.export_ui_redaction` | UI asks for an export preview | Tauri command returns a redacted package with empty Provider `apiKey`; full secret-bearing export is only written by the Save as file command | provider-import |
...
| `rc1.readiness_incomplete` | RC1 evidence is audited before marking the goal complete | `cargo xtask verify --stage rc-readiness` prints prompt-to-artifact evidence and returns non-zero until current-phase Windows real Desktop smoke, macOS arm64/x64 workflow smoke, and macOS real Desktop smoke pass evidence exist; final pass handoffs must carry collector-style fingerprint/test/log/workflow markers and P83-or-later phase markers, not only free-text summaries or old workflow evidence | rc-readiness |
...
| `gateway.unmapped_model_route` | `cargo xtask verify --stage gateway` | Covered by `/v1/messages` mapping core returning 400 with no `Default` fallback and router coverage that rejects raw-looking unmapped models without echoing them in the response or log route field |
...
| `provider.crud_reorder_roundtrip` | `cargo xtask verify --stage provider-parity`; `cargo test -p cc-desktop-switch --lib settings_port_change_refreshes_running_gateway_runtime -- --nocapture` | Covers provider edit/delete/reorder persisted roundtrip, exact-set reorder validation, active provider reassignment after delete, summary secret redaction, gateway restart on changed active provider fingerprint, and gateway listener refresh when `proxy_port` changes |
| `provider.import_export_roundtrip` | `cargo xtask verify --stage provider-import` | Covers Provider export package roundtrip, redacted UI export preview, CC-Switch legacy import, dry-run without config write, conflict blocking until replace is requested, duplicate provider ID rejection, and raw route ID rejection |
...
| `gateway.auth_required` | `cargo test -p cc-desktop-switch --lib gateway_auth -- --nocapture` and `cargo test -p cc-desktop-switch --lib gateway -- --nocapture` | P80 covers missing key, invalid key, valid `Authorization: Bearer`, valid `x-api-key`, generated config-scoped gateway key, gateway smoke bearer auth, Proxy stats/logs for auth failures, and no key values in auth error bodies or runtime log messages |
| `update.runtime_check_download_verify_install` | `cargo test -p cc-desktop-switch --lib update -- --nocapture`; `cargo test -p cc-desktop-switch --lib release_gate -- --nocapture` | P83 covers current-platform manifest parse/asset selection, same-version unavailable state, production-pinned public-key fingerprint validation with test-only fixture override, release workflow public-key fingerprint guard, required latest/selected-asset sha/signature sidecars, latest metadata sha/signature/public-key verification during both check and download before asset trust, temp-staging-before-final-replace behavior, plain-file-name rejection for manifest-controlled files, selected runtime bundle verification without requiring all release-platform assets, staging-bound installer launch revalidation, and command-backed UI actions for check/download-verify/install |
| `desktop.clear_restart_actions` | `cargo test -p cc-desktop-switch --lib desktop_writer -- --nocapture`; Windows/macOS manual smoke | P82 covers local `configLibrary` clear behavior that removes only `cc-desktop-switch-local-gateway`, preserves unrelated profiles and metadata, exposes command-backed clear/restart actions, asks for UI confirmation, and shows a restart reminder after apply. P83 blocks clear before local mutation when managed config is detected. Real restart still requires platform smoke |
| gateway Anthropic passthrough | `cargo xtask verify --stage gateway`; `cargo test -p cc-desktop-switch --lib anthropic_sse_normalizer_handles_split_frames_and_model_whitespace -- --nocapture` | Covers safe route replacement with upstream model in internal upstream request only, plus response/error/SSE normalization back to the safe route even when upstream frames are split, formatted differently, or contain raw model substrings in JSON string values |
| gateway OpenAI Chat conversion | `cargo xtask verify --stage gateway`; `cargo test -p cc-desktop-switch --lib gateway_adapter -- --nocapture` | Covers system/messages/max_tokens/temperature/stream conversion, invalid message shape as `provider.api_format_mismatch`, Anthropic `tools/tool_choice/tool_use/tool_result` -> OpenAI Chat conversion, OpenAI `tool_calls` -> Anthropic `tool_use`, and streaming `tool_calls` delta conversion |
...
| `ui.readiness_dashboard` | `Push-Location ui; trunk build --release; Pop-Location`; Playwright at `http://127.0.0.1:1421/?p41=1` | P41 browser smoke covered desktop and mobile rendering with no console errors, and the UI code binds readiness cards/list to the latest `health` snapshot without hand-written JS business logic |
| `ui.legacy_layout_parity` | `Push-Location ui; trunk build --release; Pop-Location`; Playwright at `http://127.0.0.1:1421/?p71=rerender` | P71 browser smoke covered desktop `1366x900` and mobile `390x900`, including Dashboard and Provider. Screenshots are under `target/ui-smoke/p71/`. Console result: 0 errors, 1 existing Chromium/Trunk SRI warning |
| `ui.current_frontend_parity` | `Push-Location ui; trunk build --release; Pop-Location`; Playwright at `http://127.0.0.1:1425/?p75=...` | P75 supersedes P74 for browser-smoke layout evidence. P83 code review also requires active Provider labels to use backend `active_provider`, selected rows to remain local-only, no hidden selected-row toolbar mutation without visible selection, Settings import buttons to distinguish preview from import, Add Provider static checks to say they target the current active Provider, independent model-mapping load/save to require an existing editing Provider instead of falling back to selected Provider during new drafts, Desktop/Proxy status colors to follow backend truth, and no hand-written JS business logic. Mobile is not an acceptance target for this desktop app |
...
| `rc1.readiness_incomplete` | `cargo xtask verify --stage rc-readiness` | P83 local run intentionally returns non-zero until fresh P83 macOS arm64/x64 workflow evidence and fresh P83 macOS real Desktop evidence are recorded for the current commit. The fresh macOS checks require `expected_commit`, `commit_arm64`, `commit_x64`; real Desktop smoke also requires arm64 and x86_64 pass evidence/logs. This prevents old P75 handoffs or re-stamped artifacts from marking current update/release/UI hardening as RC-ready |
...
| `platform.macos_arm64_x64_smoke_path` | `.github/workflows/rust-mainline-platform-smoke.yml`, `scripts/macos/Collect-PlatformSmokeEvidence.ps1`, and `project-docs/runbooks/macos-rust-mainline-smoke.md` | Workflow path targets `macos-14` arm64 and `macos-15-intel` x64. P75 follow-up run `25724455576` passed arm64/x64 Rust workspace gate, Leptos build, Tauri build, bundle/pkg smoke, macOS real Desktop local config smoke, and artifact upload; `project-docs/handoff/2026-05-12-p75-macos-platform-smoke-evidence-summary.md` records historical downloaded arm64/x64 artifact evidence only and is not current P83 RC evidence |
| local full gate | `cargo xtask verify --all` or focused equivalent commands | Runs fmt, workspace tests, clippy, UI release build, and Tauri build on the current platform. P83 latest full gate passed `cargo xtask verify --all` with `cargo test --workspace` reporting 153 passed and 2 ignored real Desktop smoke tests, and produced Windows MSI/NSIS bundles. Current regression coverage also includes imported route validation, `ccds-gw-*` redaction, backend `proxy_port=0` rejection, absence of the fixture-only `apply_local_config` production command, current-commit macOS evidence collector checks, dual-architecture macOS real Desktop evidence collection, disabled legacy `scripts/New-Release.ps1`, UI false-active handling, and settings port change old-listener cleanup. `cargo xtask verify --stage rc-readiness` still intentionally fails with 2 current-commit P83 macOS evidence gaps. Direct standalone UI build should be run from `ui/` as `trunk build --release` or via `cargo xtask verify --all`, not from the repository root |
