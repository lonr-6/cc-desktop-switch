...
- 顶部白色 header：左侧导入 CC Switch 配置、清除桌面版配置；中间 5 个图标 tab；右侧设置、主题、添加按钮。报告问题入口放在诊断/设置路径，不抢占主 header。
- 首页使用 provider switch-board/card 列表，不再使用 P71 的三张大状态卡作为默认基线。
- 空状态吸收 CC Switch 的居中虚线容器、主按钮和次按钮层级；provider 列表吸收 CC Switch 的紧凑 card、左侧 drag handle、provider icon 和右侧 icon action 密度。
...
P74 以后，UI 优化可以提升状态解释和错误可读性，但不能脱离当前前端的桌面布局结构、密度、按钮层级和颜色 token。P71 的旧截图式大卡片布局不再作为最新 UI 基线；P73 只覆盖了部分页面，不再视为完整视觉验收。移动端不是本桌面应用的验收目标，最多只做不明显炸版的兜底兼容。
...
“导入 CC Switch 配置 / 清除桌面版配置”位于 header 左侧，和当前前端保持一致；“报告问题”属于诊断支持路径。
